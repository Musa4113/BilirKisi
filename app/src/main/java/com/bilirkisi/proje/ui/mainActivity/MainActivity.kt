...
import com.bilirkisi.proje.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection("users").document(AuthUtil.getAuthId()).update("status", true)  // Aut dan bilgileri al. db ye ekle.
...
// Son bağlantımın zaman damgasını kaydeder (en son çevrimiçi görüldüğüm zaman)
...
//                    })
        ...
        //data'larımızı tutacağımız data modelimizi set ediyoruz
        binding = DataBindingUtil.setContentView(this, R.layout.activity_main)

        sharedViewModel = ViewModelProviders.of(this).get(SharedViewModel::class.java)
        setSupportActionBar(binding.toolbar)
        // kayıt sırasında kayıt,giriş araç çubuğunu gizle
        ...
        //geri aramalar için kaydol
        EventBus.getDefault().register(this)
        //kayıt sırasında araç çubuğunu gizle
        navController = Navigation.findNavController(this, R.id.nav_host_fragment)
        navController.addOnDestinationChangedListener { _, destination, _ ->
            if (destination.label == "SignupFragment" || destination.label == "LoginFragment") {
                binding.toolbar.visibility = View.GONE
        ...
        //navigasyonlu kurulum araç çubuğu
    ...
    // Bağlantı durumu her değiştiğinde snackbar göster
